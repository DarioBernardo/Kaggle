...
from 
test as t 
left join (select bidder_id, avg(response_time) as avg_response_time from test join auction_response_time using(bidder_id) where bid_seq_num <> 1 group by bidder_id) as tt 
using(bidder_id)
left join
(SELECT bidder_id, avg(auction_last_bid_time-time) as average_time_from_auction_last_bet, avg(auction_first_bid_time) as average_time_from_auction_first_bet FROM test join auction_response_time using(bidder_id) join auction_temp using(auction) group by bidder_id) as tlb
using(bidder_id)
left join bidder_helper as b using(bidder_id)
left join phone_helper as p using(bidder_id)
left join country_helper as c using(bidder_id)
left join url_helper as u using(bidder_id)
left join auction_helper as a using(bidder_id)
left join position_helper as ph using(bidder_id)
left join merchandise_pivoted as m using(bidder_id)
INTO OUTFILE '/tmp/test_dataset.csv'
